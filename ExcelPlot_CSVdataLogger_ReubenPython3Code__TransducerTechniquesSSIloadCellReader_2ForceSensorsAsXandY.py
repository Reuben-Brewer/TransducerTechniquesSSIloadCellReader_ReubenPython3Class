...
def CreateExcelChart(FileName_to_save_full_path, DataOrderedDictToWrite):
    ...
    SumOfForceDerivativesFromAllSensors_ExcelColumnLetter = "E"

    ForceDerivative0_ExcelColumnLetter = "F"

    ForceDerivative1_ExcelColumnLetter = "G"

    Force0_vs_Time_Xaxis_Chart_sheet = workbook.add_chartsheet("Force0_vs_Time")
    Force0_vs_Time_Xaxis_Chart = workbook.add_chart({'type': 'scatter'}) #http://xlsxwriter.readthedocs.io/example_chart_scatter.html
    Force0_vs_Time_Xaxis_Chart.add_series({'name': 'Force0_vs_Time','categories': "=Sheet1!$" + Time_ExcelColumnLetter + "$2:$" + Time_ExcelColumnLetter + "$"+str(NumberOfDataRows+1),'values': "=Sheet1!$" + Force0_ExcelColumnLetter + "$2:$" + Force0_ExcelColumnLetter + "$" + str(NumberOfDataRows+1)}) #X VALUES FIRST, THEN Y
    Force0_vs_Time_Xaxis_Chart.set_title ({'name': 'Force0 vs Time'})
    Force0_vs_Time_Xaxis_Chart.set_x_axis({'name': 'Time'})
    Force0_vs_Time_Xaxis_Chart.set_y_axis({'name': 'Force0 (lb)'})
    Force0_vs_Time_Xaxis_Chart_sheet.set_chart(Force0_vs_Time_Xaxis_Chart)

    Force1_vs_Time_Xaxis_Chart_sheet = workbook.add_chartsheet("Force1_vs_Time")
    Force1_vs_Time_Xaxis_Chart = workbook.add_chart({'type': 'scatter'}) #http://xlsxwriter.readthedocs.io/example_chart_scatter.html
    Force1_vs_Time_Xaxis_Chart.add_series({'name': 'Force1_vs_Time','categories': "=Sheet1!$" + Time_ExcelColumnLetter + "$2:$" + Time_ExcelColumnLetter + "$"+str(NumberOfDataRows+1),'values': "=Sheet1!$" + Force1_ExcelColumnLetter + "$2:$" + Force1_ExcelColumnLetter + "$" + str(NumberOfDataRows+1)}) #X VALUES FIRST, THEN Y
    Force1_vs_Time_Xaxis_Chart.set_title ({'name': 'Force1 vs Time'})
    Force1_vs_Time_Xaxis_Chart.set_x_axis({'name': 'Time'})
    Force1_vs_Time_Xaxis_Chart.set_y_axis({'name': 'Force1 (lb)'})
    Force1_vs_Time_Xaxis_Chart_sheet.set_chart(Force1_vs_Time_Xaxis_Chart)
